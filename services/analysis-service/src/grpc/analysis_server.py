...
from grpc.generated import analysis_pb2, analysis_pb2_grpc
...
class AnalysisServicer(analysis_pb2_grpc.AnalysisServiceServicer):
    ...
    def AnalyzeCode(self, request, context):
        ...
        try:
            ...
            result = loop.run_until_complete(self._analyze_code_async(request))
            loop.close()
            return result
    ...
    def GetPRAnalysis(self, request, context):
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(self._get_pr_analysis_async(request))
            loop.close()
            return result
        except Exception as e:
            context.set_code(grpc.StatusCode.INTERNAL)
    ...
    def GetAnalysisHistory(self, request, context):
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(self._get_analysis_history_async(request))
            loop.close()
            return result
        except Exception as e:
            context.set_code(grpc.StatusCode.INTERNAL)
